...
create policy "Usuarios pueden modificar sus predicciones de la Parte 1" 
  ...
  with check (
    auth.uid() = user_id 
    and get_app_time() < '2026-06-11T19:00:00Z'::timestamp with time zone -- Límite de bloqueo Parte 1
  );
...
create policy "Usuarios pueden ver predicciones de otros si están bloqueadas o son propias"
  ...
  using (
    auth.uid() = user_id
    or get_app_time() >= '2026-06-11T19:00:00Z'::timestamp with time zone
  );
...
create policy "Usuarios pueden modificar sus campeones de la Parte 1"
  ...
  with check (
    auth.uid() = user_id
    and get_app_time() < '2026-06-11T19:00:00Z'::timestamp with time zone
  );
