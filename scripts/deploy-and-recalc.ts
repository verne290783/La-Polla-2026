...
async function run() {
  ...
  try {
    console.log('\nDeploying calculate_points.sql DDL...');
    await connectedClient.query(ddlSql);
    console.log('DDL Deployed successfully!');

    console.log('\nRunning match score recalculation...');
    const result = await connectedClient.query(`
      SELECT public.compute_points(id) FROM public.matches WHERE status = 'finished';
    `);
    console.log(`Recalculated points for ${result.rowCount} matches!`);
  } catch (err: any) {
