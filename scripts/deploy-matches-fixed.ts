...
async function run() {
  try {
    ...
    // 1. Run migration
    const migrationPath = path.resolve(process.cwd(), 'migrate_matches_fixed.sql');
    const migrationSql = fs.readFileSync(migrationPath, 'utf8');
    await executeSql(sessionId, migrationSql, 'migrate_matches_fixed.sql');
    ...
    console.log('\nMIGRATION AND RECALCULATION FINISHED SUCCESSFULLY!');
  } catch (err: any) {
